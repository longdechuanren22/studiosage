...
router.get('/stats', async (req, res) => {
    await initDb();
    const userId = req.userId;
    const newCount = queryOne('SELECT COUNT(*) as c FROM messages WHERE user_id = ? AND status = ?', [userId, 'pending'])?.c || 0;
    const repliedCount = queryOne('SELECT COUNT(*) as c FROM messages WHERE user_id = ? AND status = ?', [userId, 'replied'])?.c || 0;
    const urgentCount = queryOne('SELECT COUNT(*) as c FROM messages WHERE user_id = ? AND category = ? AND status = ?', [userId, 'urgent', 'pending'])?.c || 0;
    const totalCount = queryOne('SELECT COUNT(*) as c FROM messages WHERE user_id = ?', [userId])?.c || 0;
...
router.post('/:id/send', async (req, res) => {
    ...
    if (!conn)
        return res.status(400).json({ error: '邮箱未连接，请先连接邮箱' });
    const connData = conn;
...
router.post('/cleanup-spam', async (req, res) => {
    await initDb();
    const userId = req.userId;
    const messages = queryAll("SELECT id, from_address, subject, body, client_id, status FROM messages WHERE user_id = ? AND status != 'archived'", [userId]);
    let cleaned = 0;
...
router.get('/search', async (req, res) => {
    ...
    const results = queryAll(`SELECT m.*, c.name as client_name FROM messages m
     LEFT JOIN clients c ON m.client_id = c.id
     WHERE m.user_id = ? AND m.category != 'spam' AND (m.subject LIKE ? OR m.body LIKE ?)
     ORDER BY m.created_at DESC LIMIT 20`, [userId, `%${q}%`, `%${q}%`]);
    res.json({ results });
});
