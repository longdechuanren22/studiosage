...
router.get('/', async (req, res) => {
    ...
    const pendingClients = queryOne("SELECT COUNT(DISTINCT client_id) as count FROM messages WHERE user_id = ? AND status = 'pending' AND category != 'spam' AND client_id IS NOT NULL", [userId]);
    const newMessages = queryOne("SELECT COUNT(*) as count FROM messages WHERE user_id = ? AND status = 'pending' AND category != 'spam'", [userId]);
    const urgentCount = queryOne("SELECT COUNT(*) as count FROM messages WHERE user_id = ? AND category = 'urgent' AND status = 'pending'", [userId]);
    const activeProjects = queryOne("SELECT COUNT(*) as count FROM projects WHERE user_id = ? AND status NOT IN ('completed','cancelled')", [userId]);
    // Pipeline breakdown
    const pipelineRows = queryAll("SELECT stage, COUNT(*) as count FROM clients WHERE user_id = ? AND status != 'archived' GROUP BY stage", [userId]);
    const pipeline = { inquiry: 0, engaged: 0, booked: 0, shooting: 0, production: 0, delivered: 0 };
  ...
  `, [userId]);
    ...
    const enriched = recentActivity.map((item) => ({
        ...item,
...
router.get('/analytics', async (req, res) => {
  ...
  `, [userId]);
    ...
    const totalInquiries = queryOne("SELECT COUNT(*) as c FROM clients WHERE user_id = ? AND status != 'archived'", [userId])?.c || 0;
    const totalBooked = queryOne("SELECT COUNT(*) as c FROM clients WHERE user_id = ? AND stage IN ('booked','shooting','production','delivered')", [userId])?.c || 0;
