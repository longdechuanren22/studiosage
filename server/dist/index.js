...
async function tryStartEmailWatcher() {
    try {
        ...
        await initDb();
        const connections = queryAll("SELECT * FROM tool_connections WHERE tool_id = 'email_imap' AND status = 'active'");
        ...
        logger.warn('Could not auto-start email watcher:', err.message);
    }
}
